#!/bin/bash
# Drives the meet TUI through a recording (and that nothing calls Claude while it records),
# stopping it with x and the meeting summary, session browsing, the live transcript files,
# the question session (a, meet ask), the settings modal and discarding a recording (D)
# with a fake claude (no money spent), on a private tmux server.
...
echo "== 1. first launch: idle until r; no Claude call while recording, x stops the recording, the summary lands"
# Slower, so the recording is still going when the checks below look at it.
SUGGEST_DELAY=4 launch_meet --speed 3
...
# A few lines in (the 00:24 one is the fourth), so the session keeps more than one line, and
# still well before the replay ends on its own.
wait_for "│00:24 [a-z]+ +[A-Za-z]" 20 "transcript lines land as the replay goes" || exit 1
cap | grep -q "▶ REPLAY" && ok "the replay is still going" || bad "the replay already ended"
cap | grep -q "Related" && bad "a Related pane is still drawn" || ok "no Related pane"
cap | grep -q "╭ Meeting summary" && ok "the right pane is the meeting summary while recording" || bad "right pane: $(cap | sed -n 3p)"
cap | grep -q "written when the recording stops" && ok "it says the summary is written when the recording stops" || bad "no hint in the summary pane"
cap | grep -qE "in [0-9.]+k? · out" && bad "something was spent while recording: $(cap | tail -1)" || ok "nothing spent while recording"
n_calls=$(cat "$W/claude.log" 2>/dev/null | grep -c -- '--json-schema')
[ "$n_calls" -eq 0 ] && ok "no Claude call while recording" || bad "claude was called while recording: $(grep -- '--json-schema' "$W/claude.log" | head -2)"
keys x
sleep 0.4
...
# the write-up lands in the pane.
...
cap | grep -q "✓ meet's summary: written" && ok "the pane says meet's call is done" || bad "no done line in the pane"
cap | grep -qE "^ meet .*■ ended" && ok "the header says the recording ended" || bad "header does not say ended: $(cap | sed -n 1p)"
cap | tail -1 | grep -q "summary written" && ok "the flash says the summary is written" || bad "flash: $(cap | tail -1)"
keys m
sleep 0.3
cap | grep -q "Meeting summary" && ok "m keeps the summary pane" || bad "m lost the summary pane"
n_calls=$(grep -c -- '--json-schema' "$W/claude.log")
n_suggests=$(grep -cF '"notes":{"type":"string"}' "$W/claude.log")
[ "$n_suggests" -eq 1 ] && ok "exactly one summary call, after the stop" || bad "expected 1 summary call, saw $n_suggests"
[ "$n_calls" -eq 1 ] && ok "and no other headless Claude call" || bad "expected 1 headless call, saw $n_calls"
wait_for "in [0-9.]+k? · out [0-9.]+k? · [$][0-9]+\\.[0-9]+ *$" 10 "the footer's bottom right counts the summary call's tokens and cost" || exit 1
...
cap | grep -q "Meeting summary" && ok "Tab returns to the meeting summary" || bad "Tab did not return to the summary"

echo "== 2. quit, and a second launch records a second session"
keys q
for i in $(seq 1 60); do alive || break; sleep 0.5; done
alive && { bad "meet did not exit (nothing was running, so q needs no confirm)"; tmux -L $SOCK kill-server; exit 1; } || ok "q quit at once: nothing was recording or summarizing"
start_meet
wait_for "Key points" 40 "the second recording's write-up in the Summary pane" || exit 1

echo "== 3. sessions: the list, viewing the previous one, stepping back"
keys S
sleep 0.4
wait_for "Sessions · 2 in todo" 5 "the session list with both sessions" || exit 1
cap | grep -qE "● live" && ok "the live row" || bad "no live row"
cap | grep -qE "[0-9]{4}-[0-9]{2}-[0-9]{2} [0-9]{2}:[0-9]{2} · .*lines" && ok "the previous session row with its line count" || bad "previous session row"
keys j
sleep 0.2
keys Enter
wait_for "session 2 of 2" 5 "the header shows the viewed session" || exit 1
cap | grep -q "Fake meeting 1" && ok "the session opens on its own write-up" || bad "the past session's Summary pane lacks its write-up"
...
echo "== 5. the live transcript files, and a: Claude Code on the embedded terminal in the right pane"
...
[ "$n_dirs" -ge 2 ] && ok "$n_dirs session directories with live files" || bad "expected a session dir per launch, saw $n_dirs"
# The newest live meeting's files — not the past session's, which `a` rewrote from the
# database in step 3 (that summary.md carries no write-up).
live_t=$(ls -t "$W"/data/sessions/*/transcript.md | grep -v "^$past_dir/" | head -1)
...
grep -qi "action items" "$live_s" && bad "summary.md still mentions action items" || ok "summary.md has no action items"
...
keys Tab
sleep 0.4
cap | grep -q "╭ Meeting summary" && ok "Tab cycles on to the meeting summary" || bad "Tab did not leave the bar for the summary"
...
echo "== 6. meet ask from a plain shell picks the newest meeting and execs claude"
...
echo "== 7. the settings modal: a change lands in settings.json and the next summary runs with it"
start_meet --speed 3
wait_for "▶ REPLAY" 20 "the replay is going" || exit 1
keys ,
wait_for "Settings" 5 "the settings modal" || exit 1
cap | grep -q "\[sonnet\]" && ok "the summary model row shows its default" || bad "no [sonnet] row"
cap | grep -qF "data/settings.json" && ok "the modal names the settings file" || bad "the settings file is not named"
keys l
wait_for "saved · summary model opus" 5 "the notice says the change was saved" || exit 1
grep -q '"model": "opus"' "$W/data/settings.json" && ok "settings.json carries the new summary model" || bad "settings.json: $(cat "$W/data/settings.json" 2>&1)"
keys j
sleep 0.3
keys l
wait_for "saved · summary effort low" 5 "the effort row cycles too" || exit 1
keys Escape
sleep 0.4
cap | grep -q " Settings " && bad "Esc did not close the settings" || ok "Esc closed the settings"
# The replay ends on its own; its summary call is the first to see the new settings.
for i in $(seq 1 80); do grep -qE -- "--model opus --effort low" "$W/claude.log" && break; sleep 0.5; done
grep -qE -- "--model opus --effort low" "$W/claude.log" && ok "the summary call ran with --model opus --effort low" || bad "no summary call with the new model and effort"
keys ,
sleep 0.4
keys R
...
start_meet --suggest-model haiku
wait_for "REPLAY|ended" 20 "meet is up" || exit 1
keys ,
wait_for "suggest-model haiku this launch" 5 "the modal names the flag that overrides the row" || exit 1
keys Escape
sleep 0.3
keys q; sleep 0.3; keys y
for i in $(seq 1 60); do alive || break; sleep 0.5; done
...
echo "== 8. D discards the recording: nothing is kept, nothing is summarized, meet quits"
...
n_suggests_before=$(grep -cF '"notes":{"type":"string"}' "$W/claude.log")
start_meet --speed 3
wait_for "▶ REPLAY" 20 "the replay is going" || exit 1
...
cap | grep -q "no summary is written" && ok "it says nothing is written" || bad "discard confirm copy"
keys n
sleep 0.4
...
n_suggests_after=$(grep -cF '"notes":{"type":"string"}' "$W/claude.log")
[ "$n_suggests_after" -eq "$n_suggests_before" ] && ok "no summary call was made" || bad "a summary call ran after the discard"
tmux -L $SOCK kill-server 2>/dev/null

echo "== 9. r after a recording ended starts the next one as a new session"
...
cap | grep -q "written when the recording stops" && ok "the summary pane says what happens when this one stops" || bad "summary pane after r: $(cap | sed -n 3,6p)"
